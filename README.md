...
|---|---|
| **Hero** ⭐ | A scroll-driven **real-time 3D** sequence (Three.js): assembled bracelet ring spins → zooms into a bead → **explodes it** to reveal the gold symbol plate, PCB, LED + ERM motor → closes → spins to the **core hub** → explodes it to reveal the core PCB (BLE module), battery and speaker → closes → settles with the CTA. Real CAD shells (`akoma_parts.glb`, converted from the Fusion STLs) + proxy internals + a braided cord. Falls back to the `makoma-hero.jpg` render on mobile / no-WebGL / reduced-motion. |
